...
必须遵守：
1. 只输出一个合法 JSON 对象，不要 Markdown、代码围栏、解释或前后缀。
2. 输出字段必须是：overall_score、dimension_analysis、strengths、improvements、evidence、limitations、disclaimer。
3. overall_score 必须由下列八个维度按权重综合得出，范围为 0 到 1。
...
6. 每个评分维度必须有 summary 和 evidence；evidence 必须来自用户回答文本或多模态报告的明确描述。
...
2. 只输出 summary、strengths、priority_improvements、practice_plan、dimension_analysis、limitations、disclaimer。
...
9. dimension_analysis 必须恰好包含上述八项，并分别总结多题证据；证据不足时使用 null 分数并写明限制。
"""
