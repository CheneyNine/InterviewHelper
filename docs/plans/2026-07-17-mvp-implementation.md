...
**Architecture:** 单仓库中由 Web 调用 Core API；Core API 持久化状态并编排两个内部 AI 服务。模块通过版本化 HTTP/JSON 契约隔离，先用 Stub 完成联调，再逐一替换真实模型。

**Tech Stack:** Next.js/TypeScript、FastAPI/Python、SQLAlchemy、SQLite→PostgreSQL、HTTPX、ffmpeg、Docker Compose。
...
- Create: `apps/web/src/app/interviews/[id]/page.tsx`
- Create: `apps/web/src/app/interviews/[id]/report/page.tsx`
...
6. 提交 `feat(web): complete stub interview flow`。
...
**Files:**
- Create: `apps/web/src/components/AnswerRecorder.tsx`
- Create: `apps/web/src/hooks/useMediaRecorder.ts`
...
3. 实现浏览器录制、重录、本地预览和 multipart 上传。
4. 用 Chrome 生成的真实 WebM 跑上传测试。
