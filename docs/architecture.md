...
```mermaid
flowchart LR
    APP["Mac App：JD、录制、报告"] -->|Public REST| API["Core API：业务入口与编排"]
    API --> DB[(PostgreSQL)]
...
负责 macOS 权限、录制、上传进度、轮询状态和展示。它不保存模型密钥、不拼装最终报告、不直接访问服务器文件系统或推理服务。

推荐技术栈：Tauri 2 + React + Vite + TypeScript + TanStack Query + Zustand。Tauri 负责 macOS `.app` 打包和原生能力，React/Vite 负责界面，浏览器媒体 API 负责摄像头/麦克风录制，Rust 只在需要文件系统、系统权限或安全存储时增加命令。MVP 先交付 macOS，不做移动端。

Tauri 在 macOS 使用 WebKit WebView，因此录制要在真实 Mac 上验证；发布包必须配置 `NSCameraUsageDescription`、`NSMicrophoneUsageDescription` 和相应 entitlements。
...
1. Mac App 创建 `Interview`。
2. API 调用 Interviewer AI 生成问题，将结果写入数据库。
3. Mac App 获取问题并逐题录制。
4. Mac App 以 `multipart/form-data` 将每题媒体上传到 API。
...
- Mac App 通过轮询获取状态，MVP 不引入 WebSocket。
