...
```mermaid
flowchart LR
    WEB["Web：JD、录制、报告"] -->|Public REST| API["Core API：状态与编排"]
    API --> DB[(PostgreSQL)]
...
负责浏览器权限、录制、上传、轮询状态和展示。它不保存模型密钥、不拼装最终报告、不直接访问文件系统或推理服务。

### Core API

唯一负责用户可见 API、数据持久化、文件校验、ID、状态机、重试与结果聚合。所有跨模块调用均由它发起。
...
3. Web 获取问题并逐题录制。
4. Web 以 `multipart/form-data` 将每题媒体上传到 API。
